...
class PrecompRegionDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_name, data_split, vocab, opt, train):
        ...
        with open(osp.join(loc_cap, '%s_caps.txt' % data_split), 'r') as f:
            for line in f:
                self.captions.append(line.strip())
        # Image features
        if 'coco' in self.data_name:
            all_data_pre=np.load('/media/hdd4/luz/vse_infty-bigru_mix/data_pre/coco_pre_caffe.npy',allow_pickle=True)
        elif 'f30k' in self.data_name:
            all_data_pre = np.load('/media/hdd4/luz/vse_infty-bigru_mix/data_pre/f30k_pre_caffe.npy',allow_pickle=True)
        d = {}
        for i in range(all_data_pre.shape[0]):
            d[all_data_pre[i]['img_id']] = i
        ids = np.loadtxt(os.path.join(data_path + '_precomp', data_split+'_ids.txt'))
        if 'coco' in self.data_name:
            self.idmapping= json.load(open('./data/coco/id_mapping.json'))
        elif 'f30k' in self.data_name:
            self.idmapping = json.load(open('./data/f30k/id_mapping.json'))
        self.imgnames = [self.idmapping[f'{int(i)}'] for i in ids]
        y = [d[i] for i in self.imgnames]
        data_pre = all_data_pre[y]
        self.images = [i['features'] for i in data_pre]
        self.classes = [i['labels'] for i in data_pre]
        del all_data_pre
        del data_pre
    ...
    def __getitem__(self, index):
        ...
        # Convert caption (string) to word ids (with Size Augmentation at training time).
        target,tokens = process_caption(self.vocab, caption, self.train)
        image = self.images[img_index]
        c_raw=self.classes[img_index]
        if self.train:  # Size augmentation on region features.
            ...
            c_raw = [c_raw[i] for i in np.where(rand_list > 0.20)[0].tolist()]
        c=process_classes(self.vocab, c_raw)
        image = torch.Tensor(image)
        if 'trail' in tokens and 'forest' in tokens:
            print(tokens)
            print(target)
            print(index)
        return image, target, index, img_index,c,c_raw,tokens

    def __len__(self):
        return self.length

def process_classes(vocab, c):
    caption = list()
    caption.extend([vocab(token) for token in c])
...
def process_caption(vocab, caption, drop=False):
    if not drop:
        ...
        tokens=[vocab.idx2word[f'{i}'] for i in tokens]
        return target,tokens
...
def collate_fn(data):
    ...
    images, captions, ids, img_ids,cs,c_raw,tokens = zip(*data)
    if len(images[0].shape) == 2:  # region feature
        ...
        lengths_c=[len(i) for i in cs]
        classes=torch.zeros(len(cs), max(lengths_c)).long()
        for i, cap in enumerate(captions):
            end = lengths[i]
            targets[i, :end] = cap[:end]
        for i, cap in enumerate(cs):
            end = lengths_c[i]
            classes[i, :end] = cap[:end]
        return all_images, img_lengths, targets, lengths,classes,lengths_c,c_raw,tokens, ids
